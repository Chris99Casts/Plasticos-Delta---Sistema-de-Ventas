...
def ensure_files():
    ...
    data_dir = os.path.dirname(PRODUCTOS_PATH)
    if data_dir:
        os.makedirs(data_dir, exist_ok=True)

    # Crear productos.csv con un ejemplo si no existe
    if not os.path.exists(PRODUCTOS_PATH):
        with open(PRODUCTOS_PATH, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["producto", "precio", "precio_desc"])
            w.writerow(["Tubo PVC 1/2", "12.50", "10.80"])

    # Crear pedidos.csv vacío con encabezados si no existe
    ...
    # Crear pedidos_detalle.csv vacío con encabezados si no existe
    ...
    # Crear clientes.csv mínimo si no existe
    if not os.path.exists(CLIENTES_PATH):
        with open(CLIENTES_PATH, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["id_cliente", "cliente", "es_preferencial", "tiene_descuento"])
            w.writerow(["1", "MOSTRADOR", "0", "0"])

    # Crear pedidos_pagos.csv si no existe
    if not os.path.exists(PEDIDOS_PAGOS_PATH):
        with open(PEDIDOS_PAGOS_PATH, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["id_pago", "id_pedido", "fecha", "monto", "metodo"])
